 ...
 *
 * Jedan poziv po HTTP requestu (page + Reviews sekcija dijele rezultat).
 */
async function getGoogleReviewsImpl() {
  try {
    ...
    return normalizeShape(readFromManualFallback());
  } catch (err) {
...
export const getGoogleReviews = cache(getGoogleReviewsImpl);

/** Sprečava crash ako API vrati reviews u neočekivanom obliku. */
