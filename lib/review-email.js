...
import {
  acquireReviewEmailClaim,
  completeReviewEmailClaim,
  getReservation,
  listReservations,
  releaseReviewEmailClaim,
  updateReservation,
} from "@/lib/db";
...
export async function sendReviewEmailForReservation(
  ...
  const eligibility = getReviewEmailEligibility(current, {
    now,
    require24Hours,
  });
  if (!eligibility.eligible) {
    throw new ReviewEmailError(
      eligibility.code,
      eligibilityMessage(eligibility.code)
    );
  }

  const acquired = await acquireReviewEmailClaim(reservationId, {
    reservationId,
    claimedAt,
    source,
    status: "sending",
  ...
  if (!acquired) {
    throw new ReviewEmailError(
      "SEND_IN_PROGRESS",
      "Review email is already sent or sending."
    );
  }
  ...
  try {
    ...
    const claimed = await updateReservation(reservationId, (reservation) => {
      const latestEligibility = getReviewEmailEligibility(reservation, {
        now,
        require24Hours,
      });
      if (!latestEligibility.eligible) {
        claimStatus = latestEligibility.code;
        return reservation;
      }
    ...
    await completeReviewEmailClaim(reservationId, {
      reservationId,
      status: "sent",
      ...logEntry,
    });
    ...
    // Once Gmail accepted the message, keep the durable claim even if writing
    // the receipt fails. This favors never sending a duplicate.
    if (!emailAcceptedByProvider) {
      await Promise.allSettled([
        releaseReviewEmailClaim(reservationId),
        updateReservation(reservationId, (reservation) => ({
          ...reservation,
          reviewEmailSendingAt:
            reservation.reviewEmailSendingAt === claimedAt
              ? null
              : reservation.reviewEmailSendingAt,
        })),
      ]).then((results) => {
