...
const REVIEW_EMAIL_CLAIM_PREFIX = "mpark/review-email-claims";
...
function reviewEmailClaimPath(reservationId) {
  return `${REVIEW_EMAIL_CLAIM_PREFIX}/${encodeURIComponent(reservationId)}.json`;
}
...
export async function acquireReviewEmailClaim(reservationId, claim) {
  requireVercelStorage();
  if (!usesVercelBlob()) return true;

  const { head, put } = await import("@vercel/blob");
  const token = process.env.BLOB_READ_WRITE_TOKEN;
  const pathname = reviewEmailClaimPath(reservationId);
  ...
  try {
    ...
    // An existing immutable claim means another invocation already owns this
    // reservation. Confirm existence so network/service errors still surface.
    try {
      const existing = await head(pathname, { token });
      if (existing) return false;
    } catch (headError) {
      if (!isBlobNotFoundError(headError)) throw headError;
    }
    throw error;
  }
}

export async function completeReviewEmailClaim(reservationId, receipt) {
  requireVercelStorage();
  if (!usesVercelBlob()) return;

  const { put } = await import("@vercel/blob");
  await put(reviewEmailClaimPath(reservationId), JSON.stringify(receipt), {
    access: "private",
    addRandomSuffix: false,
    allowOverwrite: true,
    contentType: "application/json",
...
export async function releaseReviewEmailClaim(reservationId) {
  requireVercelStorage();
  if (!usesVercelBlob()) return;

  const { del } = await import("@vercel/blob");
  await del(reviewEmailClaimPath(reservationId), {
    token: process.env.BLOB_READ_WRITE_TOKEN,
  });
}
...
export async function updateReservation(id, updater) {
  ...
  return withStoreLock(async () => {
    ...
    const currentRow = sanitizeReservationRecord(currentRows[index]);
